...
/*Plugins*/
const SizeScreenBased = () => {
	return plugin(({ matchUtilities }) => {
		const props = [
			{ prop: 'width', name: 'w-screen', unit: 'vw' },
			{ prop: 'height', name: 'h-screen', unit: 'vh' },
			{ prop: 'min-width', name: 'min-w-screen', unit: 'vw' },
			{ prop: 'min-height', name: 'min-h-screen', unit: 'vh' },
		] as const;
		...
		const utilities = props.reduce((acc, { prop, name, unit }) => {
			return {
				...acc,
				[`${name}`]: (value: string) => {
					return {
						[prop]: `calc(${value} * 100${unit})`,
					};
				},
			};
		}, {});
...
const Variant = () => {
	return plugin(({ addVariant }) => {
		addVariant('child-enabled', '&:not(:has([disabled]))');
		addVariant('child-disabled', '&:has([disabled])');
		addVariant('checked', ['&:checked', '&[data-state=checked]']);
		addVariant('unchecked', ['&:not(:checked)', '&[data-state=unchecked]']);
		addVariant('child-checked', '&:has([data-state=checked])');
		addVariant('child-unchecked', '&:has([data-state=unchecked])');
	});
};
/**
		...
		addUtilities({
			'.bg-grid': {
				backgroundImage: `url("${svgToDataUri(
					`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" width="32" height="32" fill="none" stroke="#cecece20"><path d="M0 .5H31.5V32"/></svg>`,
				)}")`,
...
const fluidTypography = () => {
	return plugin(({ matchUtilities, theme }) => {
		const defaultMaxBreakpoint = theme('screens[2xl]');
		const defaultMinBreakpoint = theme('screens[xs]');
		matchUtilities(
			{
